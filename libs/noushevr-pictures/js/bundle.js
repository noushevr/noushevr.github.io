...
(function(root, document) {
	...
	var run = function run() {
		...
		var initMasonry = function initMasonry() {
			...
			var grid =
				document[getElementsByClassName]("masonry-grid")[0] || "";
			var gridItem =
				document[getElementsByClassName](gridItemClass)[0] || "";
			var msnry;
			var pckry;
		...
		var initPhotoswipe = function initPhotoswipe() {
			...
			var pswpGallery =
				document[getElementsByClassName](pswpGalleryClass) || "";
			var pswpGalleryItems =
				document[getElementsByClassName]("masonry-grid-item") || "";
			/*!
			...
			var initPhotoSwipeFromDOM = function initPhotoSwipeFromDOM(
				...
				var openPhotoSwipe = function openPhotoSwipe(
					...
					options = {
						galleryUID: galleryElement[getAttribute](
							"data-pswp-uid"
						),
						getThumbBoundsFn: function getThumbBoundsFn(index) {
							var thumbnail = items[index].el.children[0],
								pageYScroll =
									root.pageYOffset || docElem.scrollTop,
					...
					gallery.listen("beforeResize", function() {
						var dpiRatio = root.devicePixelRatio
			...
			var initScript = function initScript() {
				var arrange = function arrange(e) {
					...
					var fixUrlAll = function fixUrlAll(linkArr) {
						var fixUrl = function fixUrl(link) {
							var med = link[dataset].med || "";

							if (
								med &&
								parseLink(med).isCrossDomain &&
								!parseLink(med).hasHTTP
							) {
								link[dataset].med = med.replace(
									/^/,
									forcedHTTP + ":"
								);
							}
							...
							if (
								hrefString &&
								parseLink(hrefString).isCrossDomain &&
								!parseLink(hrefString).hasHTTP
							) {
								link.href = hrefString.replace(
									/^/,
									forcedHTTP + ":"
								);
							}
