...
imagePromise, Kamil, loadCSS, loadJsCss, Promise, QRCode, require, Tablesort,
...
(function(root, document) {
	...
	var run = function run() {
		...
		var loadUnparsedJSON = function loadUnparsedJSON(
			url,
			callback,
			onerror
		) {
			...
			x.onreadystatechange = function() {
				if (x.status === "404" || x.status === "0") {
					...
					return (
						onerror && "function" === typeof onerror && onerror()
					);
				} else if (
					x.readyState === 4 &&
					x.status === 200 &&
		...
		var truncString = function truncString(str, max, add) {
			add = add || "\u2026";
			return "string" === typeof str && str[_length] > max
				? str.substring(0, max) + add
				: str;
		};

		var fixEnRuTypo = function fixEnRuTypo(e, a, b) {
			var c = "";
			...
			if ("ru" === a && "en" === b) {
				...
				b =
					"f,dult`;pbqrkvyjghcnea[wxio]ms'.zF<DULT~:PBQRKVYJGHCNEA{WXIO}MS'>Z@#$^&|/?";
			} else {
				a =
					"f,dult`;pbqrkvyjghcnea[wxio]ms'.zF<DULT~:PBQRKVYJGHCNEA{WXIO}MS'>Z@#$^&|/?";
				b =
		...
		var prependFragmentBefore = function prependFragmentBefore(e, a) {
			if ("string" === typeof e) {
				e = document[createTextNode](e);
			}
		...
		var LoadingSpinner = (function() {
			...
			var spinner =
				document[getElementsByClassName](spinnerClass)[0] || "";
			...
			return {
				show: function show() {
					return (
						docBody[classList].contains(isActiveClass) ||
						docBody[classList].add(isActiveClass)
					);
				},
		...
		var Notifier42 = function Notifier42(annonce, timeout, elemClass) {
			...
			var clearContainer = function clearContainer(cb) {
		...
		var initNotifier42WriteComment = function initNotifier42WriteComment() {
			if ("undefined" !== typeof getHTTP && !getHTTP()) {
				return;
			}
			...
			var msgText =
				"Напишите, что понравилось, а что нет. Регистрироваться не нужно.";
			...
			var showMsg = function showMsg() {
				...
				var handleMsgObj = function handleMsgObj(ev) {
					ev.stopPropagation();
					ev.preventDefault();
		...
		var initTablesort = function initTablesort(scope) {
			var ctx = scope && scope.nodeName ? scope : "";
			var tableSortClass = "table-sort";
			var tableSort = ctx
				? ctx[getElementsByClassName](tableSortClass) || ""
				: document[getElementsByClassName](tableSortClass) || "";
				...
				if (root.Tablesort) {
					var arrange = function arrange(e) {
						...
						if (tableId) {
							...
							var caption = table
								? table[getElementsByTagName]("caption")[0] ||
								  ""
								: "";

							if (!caption) {
								var tableCaption = document[createElement](
									"caption"
								);
		...
		var handleDataSrcImageAll = function handleDataSrcImageAll() {
			...
			var arrange = function arrange(e) {
				if (verge.inY(e, 100)) {
					if (!e[classList].contains(isBindedClass)) {
						...
						if (srcString) {
							if (
								parseLink(srcString).isAbsolute &&
								!parseLink(srcString).hasHTTP
							) {
								e[dataset].src = srcString.replace(
									/^/,
									forcedHTTP + ":"
								);
		...
		var handleDataSrcIframeAll = function handleDataSrcIframeAll() {
			var ifrm =
		...
		var handleChaptersSelect = function handleChaptersSelect() {
			var _this = this;
		...
		var manageChaptersSelect = function manageChaptersSelect() {
			var chaptersSelect =
				document[getElementById]("chapters-select") || "";

			if (chaptersSelect) {
				chaptersSelect[_addEventListener](
					"change",
					handleChaptersSelect
				);
			}
		};
		...
		var handleExpandingLayerAll = function handleExpandingLayerAll() {
			...
			var isActiveClass = "is-active";
			var layer = _this[parentNode]
				? _this[parentNode].nextElementSibling
				: "";
		...
		var manageExpandingLayers = function manageExpandingLayers(scope) {
			var ctx = scope && scope.nodeName ? scope : "";
			var btnClass = "btn-expand-hidden-layer";
			var btn = ctx
				? ctx[getElementsByClassName](btnClass) || ""
		...
		addAppUpdatesLink = function addAppUpdatesLink() {
			var panel =
				document[getElementsByClassName]("panel-menu-more")[0] || "";
			var items = panel ? panel[getElementsByTagName]("li") || "" : "";
			var navigatorUserAgent = navigator.userAgent || "";
			var linkHref;

			if (
				/Windows/i.test(navigatorUserAgent) &&
				/(WOW64|Win64)/i.test(navigatorUserAgent)
			) {
				linkHref =
					"https://github.com/englishextra/englishextra-app/releases/download/v1.0.0/englishextra-win32-x64-setup.exe";
			} else if (
				/(x86_64|x86-64|x64;|amd64|AMD64|x64_64)/i.test(
					navigatorUserAgent
				) &&
				/(Linux|X11)/i.test(navigatorUserAgent)
			) {
				linkHref =
			...
			var arrange = function arrange() {
				...
				link.title =
					"" +
					(parseLink(linkHref).hostname || "") +
					" откроется в новой вкладке";
				link.href = linkHref;

				var handleAppUpdatesLink = function handleAppUpdatesLink() {
		...
		var initMenuMore = function initMenuMore() {
			var container = document[getElementById]("container") || "";
			var page = document[getElementById]("page") || "";
			var holderPanelMenuMore =
				document[getElementsByClassName]("holder-panel-menu-more")[0] ||
				"";
			var btnMenuMore =
				document[getElementsByClassName]("btn-menu-more")[0] || "";
			var panelMenuMore =
				document[getElementsByClassName]("panel-menu-more")[0] || "";
			var panelMenuMoreItems = panelMenuMore
				? panelMenuMore[getElementsByTagName]("li") || ""
				: "";
			var panelNavMenu =
				document[getElementsByClassName]("panel-nav-menu")[0] || "";
			var isActiveClass = "is-active";
			...
			var handleItem = function handleItem() {
				...
				if (
					panelNavMenu &&
					panelNavMenu[classList].contains(isActiveClass)
				) {
					panelNavMenu[classList].remove(isActiveClass);
				}
			};
			...
			var addBtnHandler = function addBtnHandler() {
				var handleBtnMenuMore = function handleBtnMenuMore(ev) {
					ev.stopPropagation();
					ev.preventDefault();
			...
			var addItemHandlerAll = function addItemHandlerAll() {
				...
				for (
					var i = 0, l = panelMenuMoreItems[_length];
					i < l;
					i += 1
				) {
		...
		var manageShareButton = function manageShareButton() {
			var btn =
				document[getElementsByClassName]("btn-share-buttons")[0] || "";
		...
		var manageVKLikeButton = function manageVKLikeButton() {
			...
			var btn =
				document[getElementsByClassName]("btn-show-vk-like")[0] || "";
			var isActiveClass = "is-active";
		...
		initDownloadAppBtn = function initDownloadAppBtn() {
			var navigatorUserAgent = navigator.userAgent || "";
			...
			if (
				...
				bgUrl =
					"url(../../libs/products/img/download_windows_app_144x52.png)";
				linkHref =
					"https://github.com/englishextra/englishextra-app/releases/download/v1.0.0/englishextra-win32-x64-setup.exe";
			} else if (
				/(x86_64|x86-64|x64;|amd64|AMD64|x64_64)/i.test(
				...
				bgUrl =
					"url(../../libs/products/img/download_linux_app_144x52.png)";
				linkHref =
					"https://github.com/englishextra/englishextra-app/releases/download/v1.0.0/englishextra-linux-x64.tar.gz";
			} else if (/IEMobile/i.test(navigatorUserAgent)) {
				bgUrl =
					"url(../../libs/products/img/download_wp_app_144x52.png)";
				linkHref =
					"https://github.com/englishextra/englishextra-app/releases/download/v1.0.0/englishextra.Windows10_x86_debug.appx";
			} else {
				if (/Android/i.test(navigatorUserAgent)) {
					bgUrl =
						"url(../../libs/products/img/download_android_app_144x52.png)";
					linkHref =
						"https://github.com/englishextra/englishextra-app/releases/download/v1.0.0/englishextra-debug.apk";
				}
			}
			...
			var arrange = function arrange() {
				var handleDownloadAppBtn = function handleDownloadAppBtn(ev) {
					ev.stopPropagation();
					ev.preventDefault();
					openDeviceBrowser(linkHref);
				};
		...
		var initDisqusOnScroll = function initDisqusOnScroll() {
			var disqusThread = document[getElementById]("disqus_thread") || "";
			var btn =
				document[getElementsByClassName]("btn-show-disqus")[0] || "";
			var locationHref = root.location.href || "";
			...
			var addHandler = function addHandler() {
				var handleDisqusButton = function handleDisqusButton(ev) {
					ev.preventDefault();
					ev.stopPropagation();
			...
			if (btn && disqusThread && disqusThreadShortname && locationHref) {
				if ("undefined" !== typeof getHTTP && getHTTP()) {
					...
					var msgText = document[
						createRange
					]().createContextualFragment(
						"<p>Комментарии доступны только в веб версии этой страницы.</p>"
					);
		...
		var initKamilAutocomplete = function initKamilAutocomplete() {
			var searchForm =
			...
			var jsonUrl =
				"../../libs/noushevr-contents-cards/json/contents.json";
			...
			var processJsonResponse = function processJsonResponse(
				...
				var handleTypoSuggestion = function handleTypoSuggestion() {
					setStyleDisplayNone(suggestionUl);
					setStyleDisplayNone(suggestionLi);
				};

				var showTypoSuggestion = function showTypoSuggestion() {
				...
				ac.renderMenu = function(ul, stance) {
					...
					var limitKamilOutput = function limitKamilOutput(e, i) {
						if (i < 10) {
					...
					while (itemsLength < 1) {
						...
						if (
							textInput.value[_length] < 3 ||
							textInput.value.match(/^\s*$/)
						) {
							handleTypoSuggestion();
						}
					...
					var truncateKamilText = function truncateKamilText(e) {
						var truncText = e.firstChild.textContent || "";
						var truncTextObj = document[createTextNode](
							truncString(truncText, 24)
						);
				...
				var handleSuggestionLi = function handleSuggestionLi(ev) {
					ev.stopPropagation();
					ev.preventDefault();
					textInput.focus();
				...
				ac.on("kamilselect", function(e) {
					...
					var handleKamilItem = function handleKamilItem() {
