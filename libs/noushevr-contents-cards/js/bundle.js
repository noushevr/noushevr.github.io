...
(function(root, document) {
	...
	var run = function run() {
		...
		var observeMutations = function observeMutations(scope) {
			var context = scope && scope.nodeName ? scope : "";
			var mo;

			var getMutations = function getMutations(e) {
				var triggerOnMutation = function triggerOnMutation(m) {
					...
					console.log(
						m.type,
						"added: " + m.addedNodes[_length] + " nodes"
					);
					console.log(
						m.type,
						"removed: " + m.removedNodes[_length] + " nodes"
					);
		...
		var countObjKeys = function countObjKeys(obj) {
			var count = 0;
		...
		var generateCardGrid = function generateCardGrid(text) {
			return new Promise(function(resolve, reject) {
				...
				try {
					...
					if (!jsonObj.pages[0][jsonHrefKeyName]) {
						throw new Error(
							"incomplete JSON data: no " + jsonHrefKeyName
						);
					} else if (!jsonObj.pages[0][jsonSrcKeyName]) {
						throw new Error(
							"incomplete JSON data: no " + jsonSrcKeyName
						);
					} else if (!jsonObj.pages[0][jsonWidthKeyName]) {
						throw new Error(
							"incomplete JSON data: no " + jsonWidthKeyName
						);
					} else if (!jsonObj.pages[0][jsonHeightKeyName]) {
						throw new Error(
							"incomplete JSON data: no " + jsonHeightKeyName
						);
					} else if (!jsonObj.pages[0][jsonTitleKeyName]) {
						throw new Error(
							"incomplete JSON data: no " + jsonTitleKeyName
						);
					} else {
						if (!jsonObj.pages[0][jsonTextKeyName]) {
							throw new Error(
								"incomplete JSON data: no " + jsonTextKeyName
							);
						}
					}
				} catch (err) {
				...
				insertFromTemplate(
					...
					function() {
						if (
							wrapper[getElementsByClassName](minigridItemClass)[
								pagesKeysNumber - 1
							]
						) {
							resolve();
		...
		var addCardWrapCssRule = function addCardWrapCssRule() {
			var toDashedAll = function toDashedAll(str) {
		...
		var createGrid = function createGrid() {
			...
			var onMinigridCreated = function onMinigridCreated() {
			...
			var updateMinigrid = function updateMinigrid() {
				if (mgrid) {
		...
		var setLazyloading = function setLazyloading() {
    ...
    if (wrapper && titleBar) {
		...
		var hideTitleBar = function hideTitleBar() {
			var logic = function logic() {
				titleBar[classList].remove(isFixedClass);
		...
		var revealTitleBar = function revealTitleBar() {
			var logic = function logic() {
				titleBar[classList].remove(isHiddenClass);
		...
		var resetTitleBar = function resetTitleBar() {
			var logic = function logic() {
				if (
					(document[body].scrollTop || docElem.scrollTop || 0) <
					titleBarHeight
		...
		if (titleBar) {
			root[_addEventListener]("scroll", resetTitleBar, {
				passive: true
			});

			if (hasTouch) {
				if (root.tocca) {
					document[_addEventListener]("swipeup", hideTitleBar, {
						passive: true
					});

					document[_addEventListener]("swipedown", revealTitleBar, {
						passive: true
					});
				}
			} else {
		...
		var handleBtnTotop = function handleBtnTotop(evt) {
			evt.stopPropagation();
			evt.preventDefault();
			scroll2Top(0, 20000);
		};

		var handleBtnTotopWindow = function handleBtnTotopWindow(_this) {
			var logic = function logic() {
		...
		if (btnTotop) {
			...
			root[_addEventListener]("scroll", handleBtnTotopWindow, {
				passive: true
			});
		}
	};
